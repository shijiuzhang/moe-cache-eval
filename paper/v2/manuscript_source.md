...
> `E(s, l) = union_{r in A(s)} topk(r, l, s)`, the distinct expert blocks that step
...
event boundary, so no member of `E intersect C` can be evicted before the event that
...
can evict a block in `E intersect C` that the *same* event has not yet consumed, which
...
experts, top-16, 82,432 expert blocks of approximately 16.74 MiB, approximately 106.55 GiB of non-routed
weights, approximately 1,347.12 GiB of routed expert weights, and approximately 25.83 GB of logical expert
...
| mechanism | model | $\rho$ | concurrency | scheduling | other constraints | criterion | measured | outcome in scope |
|---|---|---:|---|---|---|---|---:|---|
| steady-state prefill eviction / admission | 40- and 64-expert | 20/30/40% | prefill, 128-token blocks | — | strictly lossless | gap $\geq$ 10% | 0.155–3.169% | not met |
| warm-cache workload transition | 40- and 64-expert | 20/30/40% | prefill, 128-token blocks | directed A$\rightarrow$B transitions | windows N = 10/25/50/100 | gap $\geq$ 10% | 1439/1440 units < 10% | not met |
| semantic category partitioning | 128-expert | 40% | B = 8 | FCFS | equal total capacity; **6 slots < top-`k` = 8** | transfer $\downarrow$ $\geq$ 5% | **+115.9%** | invalid test of mechanism; design floor dominated |
| causal affinity batching | 128-expert | 40% | B = 8, 24 admitted | deadline rotation | W = 4, strictly lossless | transfer $\downarrow$ $\geq$ 10% | 4.76% / 4.55% | not met |
