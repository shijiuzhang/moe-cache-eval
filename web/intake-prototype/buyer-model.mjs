...
export function validateBuyerIntake(draft) {
  ...
  const task = (owner, code, message, blocks_freeze = true, buyer_emphasis = false) => tasks.push({ owner, code, message, blocks_freeze, buyer_emphasis });
  ...
  if (positive(draft.experience?.stability_hours)) {
    task("oicap_compiler", "SOAK_PLAN_REQUIRED", "根据已声明的连续稳定运行时长生成 soak 阶段，并定义服务退出、重启、超时和错误统计。", false, risks.includes("stability"));
  }
  if (draft.experience?.recovery_expectation) {
    ...
    task("oicap_compiler", "RECOVERY_OBSERVATION_PLAN_REQUIRED", "根据已声明的恢复要求生成中断、重启或恢复观察计划。", false, risks.includes("errors"));
  }
